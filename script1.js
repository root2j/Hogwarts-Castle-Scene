...
let params = {
    ...
    cameraInitialY: 0.5,
    cameraInitialZ: 10,
    enableHorizontalSpin: true, // Add a parameter to control horizontal spinning
...
const modelColors = {
    ...
    6: { color: 0x7E99A3, opacity: 1, emissive: 0x7E99A3, emissiveIntensity: 1 }, //Roofs - Make it shine
    7: { color: 0xFFEB00, opacity: 1, emissive: 0xFFEB00, emissiveIntensity: 20 }, //Illuminated Windows (candle yellow)
    8: { color: 0xFFEB00, opacity: 1, emissive: 0xFFEB00, emissiveIntensity: 20 }, //Illuminated Windows (candle yellow)
    9: { color: 0xFFEB00, opacity: 1, emissive: 0xFFEB00, emissiveIntensity: 20 }, //Illuminated Windows (candle yellow)
    10: { color: 0xB17457, opacity: 1 }, //Window Railings
    11: { color: 0xE8F9FF, opacity: 1 }, //Green Houses
    13: { color: 0xFFEB00, opacity: 1, emissive: 0xFFCC33, emissiveIntensity: 10 }, //Illuminated Windows (candle yellow)
...
function setupStars() {
    ...
    for (let i = 0; i < 10000; i++) {
        ...
        // Ensure stars appear above the castle by setting a higher minimum y (e.g., starting at 50)
        const y = THREE.MathUtils.randFloat(-50, 1000);
    ...
    models.forEach((model, index) => {
        ...
        objLoader.load(
            ...
            (object) => {
                ...
                // Scale model number 16 in x and z directions
                if (index == 15) {
                    object.scale.x = 10; // Adjust the x scale as needed
                    object.scale.y = 10; // Adjust the z scale as needed
                    // object.position.y -= 9;
                }
            },
            (xhr) => {
...
function setupLights() {
    const directionalLight = new THREE.DirectionalLight(0x2f4eae, 4);
...
function animate(setup, lights, composer) {
    requestAnimationFrame(() => animate(setup, lights, composer));
